...
class RegisterMedicalStorePage extends StatefulWidget {
  const RegisterMedicalStorePage({super.key});

  @override
  State<RegisterMedicalStorePage> createState() =>
      _RegisterMedicalStorePageState();
}

class _RegisterMedicalStorePageState extends State<RegisterMedicalStorePage> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Medical Store registered successfully!')),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Medical Store Registration")),
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              const Text('Two quick steps to list your health resources'),
              ...
              _buildTextFieldWithLabel(
                  'Website', websiteController, isRequired: false),
              ...
              const SizedBox(height: 16),
              const Text('HMIS Registered',
                  style: TextStyle(fontWeight: FontWeight.w500)),
              _buildHmisRadioRow(),

              const Divider(thickness: 1.5),
              const SizedBox(height: 10),
              const Text("Officer Details",
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),

              _buildTextFieldWithLabel(
                  'Officer\'s Name*', officerNameController),
              _buildTextFieldWithLabel('Designation*', designationController),
              _buildTextFieldWithLabel('Mobile Number*', mobileNumberController,
                  inputType: TextInputType.phone),
              _buildTextFieldWithLabel(
                  'Landline Number', landlineNumberController,
                  isRequired: false, inputType: TextInputType.phone),
              _buildTextFieldWithLabel('Email*', emailController,
                  inputType: TextInputType.emailAddress),
              _buildTextFieldWithLabel(
                  'Head of Organization*', headOfOrgController),
              ...
              ElevatedButton(
                ...
                child: const Text(
                    'Choose File', style: TextStyle(color: Colors.black)),
              ),
              const SizedBox(height: 10),
              if (_pickedImageMobile != null)
                Image.file(_pickedImageMobile!, height: 150,
                    width: 150,
                    fit: BoxFit.cover)
              else
                if (_pickedImageWeb != null)
                  FutureBuilder(
            ...
            builder: (fieldState) {
              return Column(
                ...
                children: [
                  ...
                  _buildTextField(
                      controller, inputType: inputType, isRequired: isRequired),
                ],
              );
            },
          ),
  ...
  Widget _buildDropdown({required String value, required List<
      String> items, required ValueChanged<String?> onChanged}) {
